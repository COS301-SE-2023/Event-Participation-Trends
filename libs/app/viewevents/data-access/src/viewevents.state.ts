...
import {
  ...
  SetSearchValue,
  SetSubscribedEvents,
  SetUnsubscribedEvents,
  SetViewEvents,
} from '@event-participation-trends/app/viewevents/util';
...
export class VieweventsState {
  ...
  setViewEvents(
    ctx: StateContext<VieweventsStateModel>,
    { newState }: SetViewEvents
  ) {
    ctx.patchState({
  ...
  setSubscribedEvents(
    ctx: StateContext<VieweventsStateModel>,
    { updatedEvents }: SetSubscribedEvents
  ) {
    try {
      const state = ctx.getState();
  ...
  setUnsubscribedEvents(
    ctx: StateContext<VieweventsStateModel>,
    { updatedEvents }: SetUnsubscribedEvents
  ) {
    try {
      const state = ctx.getState();
  ...
  async getAllEvents(ctx: StateContext<VieweventsStateModel>) {
    try {
      const response: Observable<IGetAllEventsResponse> =
        await this.appApiService.getAllEvents();
  ...
  async getSubscribedEvents(ctx: StateContext<VieweventsStateModel>) {
    try {
      const response: Observable<IGetAllEventsResponse> =
        await this.appApiService.getSubscribedEvents();
  ...
  async getMyEvents(ctx: StateContext<VieweventsStateModel>) {
    try {
      const response: Observable<IGetAllEventsResponse> =
        await this.appApiService.getManagedEvents();
  ...
  async addNewlyCreatedEvent(
    ctx: StateContext<VieweventsStateModel>,
    { newEvent }: AddNewlyCreatedEvent
  ) {
    try {
      const state = ctx.getState();
      const my_events = state.my_events as any[];
