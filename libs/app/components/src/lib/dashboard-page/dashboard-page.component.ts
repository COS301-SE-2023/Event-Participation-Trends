...
import { Component, OnInit, ViewChild, ElementRef, HostListener, AfterViewInit, NgZone, OnDestroy } from '@angular/core';
...
import { matFilterCenterFocus, matZoomIn, matZoomOut, matDragHandle } from "@ng-icons/material-icons/baseline";
...
@Component({
  ...
  providers: [
    provideIcons({heroUserGroupSolid, heroBackward, matKeyboardDoubleArrowUp, matKeyboardDoubleArrowDown, matFilterCenterFocus, matZoomIn, matZoomOut, matDragHandle})
  ]
})
export class DashboardPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Chart
  increasedUserCount = false;
  noIncreaseOrDecrease = true;

  devicesBarChart: Chart | null = null;
  streamingUserCountChart: Chart | null = null;
  eventHours: string[] = []; // labels for the chart
  userDetectedPerHour: {date: string, time: string, detected: number}[] = []; // data for the chart
  ...
  detectionRadius = 2;
  streamingInterval: any;
  loadingText = 'Fetching floor plan data';
  // ====================================
  ...
  async ngAfterViewInit() {
    ...
    const checkDOM = setInterval(() => {
      ...
      if (document.getElementById('view-' + this.id) && document.getElementById('floormap-' + this.id)) {
        ...
        setTimeout(() => {
          ...
          // this.eventStartTime.setHours(this.eventStartTime.getHours() - 5);
          // this.eventEndTime = new Date();
          // this.eventEndTime.setHours(this.eventEndTime.getHours() + 2);
          //---------------

          // Calculate the difference in hours between start and end dates
          const hoursDifference = Math.abs(this.eventEndTime.getTime() - this.eventStartTime.getTime()) / 36e5; // 36e5 is the number of milliseconds in an hour

          // Loop through the hours and create labels
          for (let i = 0; i <= hoursDifference; i++) {
            const currentHour = new Date(this.eventStartTime);
            currentHour.setHours(this.eventStartTime.getHours() + i);
          ...
          if (now > this.eventEndTime) {
            ...
            if (positions) {
              ...
              if (this.totalUsersDetectedPrev > this.totalUsersDetected) {
                ...
                this.noIncreaseOrDecrease = false;
              }
              else if (this.totalUsersDetectedPrev < this.totalUsersDetected) {
                this.increasedUserCount = true;
                this.noIncreaseOrDecrease = false;
              }
              else {
                this.increasedUserCount = false;
                this.noIncreaseOrDecrease = true;
              }
              ...
              this.userDetectedPerHour.forEach((hour) => {
                // Extract the hour and date parts from the existing hour's time and newTime
                const existingHourTime = hour.time.substring(0,2); // Extract the hour time
                const existingHourDate = hour.date; // Extract the date
              
                const newHourDate = now.toDateString(); // Extract the date
              
                // Check if both the hour and date match
                if (
                  existingHourTime === newTime.substring(0,2) && // Check if the time part matches
                  existingHourDate === newHourDate // Check if the date part matches
                ) {
